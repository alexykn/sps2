...
mod tests {
    ...
    fn test_metric_stats_calculation() {
        let mut series = TimeSeries::new(MetricType::Gauge);

        // Add some values
        let now = Instant::now();
        series.add_point(now, 10.0);
        series.add_point(now, 20.0);
        series.add_point(now, 30.0);
        series.add_point(now, 40.0);
        series.add_point(now, 50.0);

        let stats = series.calculate_stats().unwrap();
        assert_eq!(stats.count, 5);
        assert_float_eq(stats.min, 10.0);
        assert_float_eq(stats.max, 50.0);
        assert_float_eq(stats.mean, 30.0);
        assert_float_eq(stats.sum, 150.0);
    }
    ...
    fn test_percentile_calculation() {
        ...
        assert_float_eq(percentile(&values, 0.50), 5.0);
        assert_float_eq(percentile(&values, 0.90), 9.0);
        assert_float_eq(percentile(&values, 1.00), 10.0);
    }
}

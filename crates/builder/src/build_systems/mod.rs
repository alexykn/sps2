...
mod tests {
    ...
    async fn test_get_by_name() {
        ...
        assert!(registry.get("autotools").is_some());
    ...
    async fn test_autotools_detection() {
        ...
        let configure_path = temp.path().join("configure");
        fs::write(&configure_path, "#!/bin/sh\n").await.unwrap();

        let system = detect_build_system(temp.path()).await.unwrap();
        assert_eq!(system.name(), "autotools");
    }

    #[tokio::test]
    async fn test_cmake_detection() {
        let temp = tempdir().unwrap();
        let cmake_path = temp.path().join("CMakeLists.txt");
        fs::write(&cmake_path, "project(test)\n").await.unwrap();

        let system = detect_build_system(temp.path()).await.unwrap();
        assert_eq!(system.name(), "cmake");
    }

    #[tokio::test]
    async fn test_no_build_system_detection() {
        let temp = tempdir().unwrap();
        let result = detect_build_system(temp.path()).await;

...
mod tests {
    ...
    async fn test_cargo_detection() {
        ...
        // No Cargo.toml - should not detect
    ...
    fn test_build_args() {
        ...
        let args = system.get_build_args(&ctx, &["--features=async,tls".to_string()]);

        assert!(args.contains(&"build".to_string()));
        assert!(args.contains(&"--release".to_string()));
        assert!(args.contains(&"--offline".to_string()));
        assert!(args.contains(&"--features".to_string()));
        assert!(args.contains(&"async,tls".to_string()));
    }
    ...
    fn test_parse_test_summary() {
        assert_eq!(
            ...
            Some((15, 10, 2)) // total=15 (10+2+3), passed=10, failed=2
        );
        assert_eq!(
            parse_cargo_test_summary(
                "test result: FAILED. 0 passed; 1 failed; 0 ignored; 0 measured; 0 filtered out"
            ),
            Some((1, 0, 1)) // total=1 (0+1+0), passed=0, failed=1
        );
        assert_eq!(parse_cargo_test_summary("Invalid line"), None);
    }
    ...
    fn test_extract_features() {
        let system = CargoBuildSystem::new();
        let args = vec![
            "--features=async,tls".to_string(),
            "--release".to_string(),
            "--features=compression".to_string(),
        ];

        let features = system.extract_features(&args);
        assert_eq!(features, vec!["async", "tls", "compression"]);
    }
}

...
mod tests {
    ...
    #[tokio::test]
    async fn test_nodejs_detection() {
    ...
    async fn test_package_manager_detection() {
        ...
        match system.detect_package_manager(temp.path()).await {
            Ok(PackageManager::Npm) => {}
            _ => panic!("Expected npm as default"),
        }

        // Detect yarn
        fs::write(temp.path().join("yarn.lock"), "").await.unwrap();
        match system.detect_package_manager(temp.path()).await {
            Ok(PackageManager::Yarn) => {}
            _ => panic!("Expected yarn"),
        }

        // Detect pnpm
        fs::remove_file(temp.path().join("yarn.lock"))
            .await
            .unwrap();
...
";
        ...
        let (total, passed, failed, _) = system.parse_test_output(output);
        assert_eq!(total, 3);
        assert_eq!(passed, 2);
        assert_eq!(failed, 1);
    }

...
mod tests {
    ...
    #[test]
    fn test_platform_from_triple() {
        let platform = Platform::from_triple("aarch64-apple-darwin").unwrap();
        assert_eq!(platform.arch, "aarch64");
        assert_eq!(platform.vendor, Some("apple".to_string()));
        assert_eq!(platform.os, "darwin");
        assert_eq!(platform.abi, None);

        let platform = Platform::from_triple("x86_64-unknown-linux-gnu").unwrap();
        assert_eq!(platform.arch, "x86_64");
        assert_eq!(platform.vendor, Some("unknown".to_string()));
        assert_eq!(platform.os, "linux");
        assert_eq!(platform.abi, Some("gnu".to_string()));

        let platform = Platform::from_triple("armv7-linux-gnueabihf").unwrap();
        assert_eq!(platform.arch, "armv7");
        assert_eq!(platform.vendor, None);
        assert_eq!(platform.os, "linux");
        assert_eq!(platform.abi, Some("gnueabihf".to_string()));
    }

    #[test]
    fn test_invalid_triple() {
        assert!(Platform::from_triple("invalid").is_err());
        assert!(Platform::from_triple("").is_err());
    }

    #[test]
    fn test_multiarch_triplet() {
        let temp = tempdir().unwrap();
        let ctx = EnhancedCrossContext {
            base: CrossCompilationContext {
                build_platform: Platform::from_triple("x86_64-unknown-linux-gnu").unwrap(),
                host_platform: Platform::from_triple("aarch64-unknown-linux-gnu").unwrap(),
                target_platform: None,
                sysroot: temp.path().to_path_buf(),
                toolchain: Toolchain {
                    cc: "aarch64-linux-gnu-gcc".to_string(),
                    cxx: "aarch64-linux-gnu-g++".to_string(),
                    ar: "aarch64-linux-gnu-ar".to_string(),
                    strip: "aarch64-linux-gnu-strip".to_string(),
                    ranlib: "aarch64-linux-gnu-ranlib".to_string(),
                    cmake_toolchain_file: temp.path().join("toolchain.cmake"),
                    meson_cross_file: temp.path().join("cross.ini"),
                },
            },
            validated_tools: HashMap::new(),
            multiarch_paths: Vec::new(),
        ...
        assert_eq!(ctx.get_multiarch_triplet(), "aarch64-linux-gnu");
    }

    #[test]
    fn test_platform_compatibility() {
        let linux = Platform::from_triple("x86_64-unknown-linux-gnu").unwrap();
        let darwin = Platform::from_triple("aarch64-apple-darwin").unwrap();
        let windows = Platform::from_triple("x86_64-pc-windows-gnu").unwrap();

        // Linux to Windows is OK
        assert!(EnhancedCrossContext::validate_platform_compatibility(&linux, &windows).is_ok());

        // Darwin to Linux is OK
        assert!(EnhancedCrossContext::validate_platform_compatibility(&darwin, &linux).is_ok());

        // Linux to Darwin is not supported
        assert!(EnhancedCrossContext::validate_platform_compatibility(&linux, &darwin).is_err());

        // Same platform should error (not cross-compilation)
        assert!(EnhancedCrossContext::validate_platform_compatibility(&linux, &linux).is_err());
    }
}

...
mod tests {
    ...
    #[test]
    fn test_extended_dep_kind_conversion() {
        assert_eq!(ExtendedDepKind::Build.to_standard(), DepKind::Build);
        assert_eq!(ExtendedDepKind::Host.to_standard(), DepKind::Build);
        assert_eq!(ExtendedDepKind::Target.to_standard(), DepKind::Runtime);
        assert_eq!(ExtendedDepKind::Runtime.to_standard(), DepKind::Runtime);
    }

    #[test]
    fn test_dependency_with_features() {
        let spec = PackageSpec::parse("openssl>=1.0.0").unwrap();
        let dep = Dependency::new(spec, ExtendedDepKind::Build)
            .with_features(vec!["tls".to_string(), "crypto".to_string()]);

        assert!(dep.optional);
        assert_eq!(dep.features.len(), 2);
        assert!(dep.features.contains("tls"));
        assert!(dep.features.contains("crypto"));
    }

    #[test]
    fn test_dependency_context() {
        let mut ctx = DependencyContext::new("x86_64".to_string());
        ctx.enable_feature("tls");

        let spec1 = PackageSpec::parse("openssl>=1.0.0").unwrap();
        let dep1 = Dependency::new(spec1, ExtendedDepKind::Build);

        let spec2 = PackageSpec::parse("rustls>=0.20.0").unwrap();
        let dep2 =
            Dependency::new(spec2, ExtendedDepKind::Build).with_features(vec!["tls".to_string()]);

        ctx.add_dependency(dep1);
        ctx.add_dependency(dep2);

        let active = ctx.active_dependencies();
        assert_eq!(active.len(), 2); // Both should be active
    }
    ...
    fn test_topological_sort() {
        ...
        graph.add_edge("c-1.0.0", "d-1.0.0");
        ...
        let a_pos = order.iter().position(|x| x == "a-1.0.0").unwrap();
        let b_pos = order.iter().position(|x| x == "b-1.0.0").unwrap();
        let c_pos = order.iter().position(|x| x == "c-1.0.0").unwrap();
        let d_pos = order.iter().position(|x| x == "d-1.0.0").unwrap();
    ...
    fn test_dot_generation() {
        ...
        graph.add_edge("myapp-1.0.0", "libfoo-2.1.0");

        let dot = graph.to_dot();
        assert!(dot.contains("digraph Dependencies"));
        assert!(dot.contains("myapp"));

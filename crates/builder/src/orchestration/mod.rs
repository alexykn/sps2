...
mod tests {
    ...
    fn test_resource_manager() {
        ...
        assert!(resources.try_allocate(&req));
        assert!(resources.utilization() > 0.0);

        // Test release
        resources.release(&req);
        assert_float_eq(resources.utilization(), 0.0);
    }

    #[tokio::test]
    async fn test_build_scheduler() {
        let resources = Arc::new(ResourceManager::new());
        let scheduler = BuildScheduler::new(resources);
        ...
        let task2 = BuildTask::new(
            "pkg2".to_string(),
            Version::parse("2.0.0").unwrap(),
            context,
            Priority::High,
        )
        .with_dependencies(vec!["pkg1-1.0.0".to_string()]);

        // Schedule tasks
        scheduler.schedule(task1);
        scheduler.schedule(task2);

        // Task 1 should be available, task 2 should wait
        assert!(scheduler.get_next_task().is_some());
        assert!(scheduler.get_next_task().is_none());

        // Complete task 1
        scheduler.complete_task("pkg1-1.0.0", BuildResult::new(temp.path().join("pkg1.sp")));

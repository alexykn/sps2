...
mod tests {
    ...
    #[tokio::test]
    async fn test_hermetic_config_default() {
        let config = HermeticConfig::default();
        assert!(config.allowed_env_vars.contains("PATH"));
        assert!(config.allowed_env_vars.contains("HOME"));
        assert!(!config.allow_network);
        assert!(config.temp_home.is_none());
    }

    #[tokio::test]
    async fn test_clear_environment_vars() {
        ...
        let mut env = BuildEnvironment::new(context, temp.path()).unwrap();
        let config = HermeticConfig::default();

        // Set some test environment variables
        std::env::set_var("TEST_VAR_SHOULD_BE_REMOVED", "value");
        std::env::set_var("PATH", "/usr/bin:/bin");

        env.clear_environment_vars(&config).unwrap();

        // Verify only allowed vars are present
        assert!(!env.env_vars.contains_key("TEST_VAR_SHOULD_BE_REMOVED"));
        assert!(env.env_vars.contains_key("PATH"));
        assert!(env.env_vars.contains_key("PREFIX"));
        assert!(env.env_vars.contains_key("JOBS"));

        // Clean up
        std::env::remove_var("TEST_VAR_SHOULD_BE_REMOVED");
    }
    ...
    async fn test_network_isolation() {
        ...
        let mut env = BuildEnvironment::new(context, temp.path()).unwrap();
        env.apply_network_isolation().unwrap();

        // Verify proxy variables are set
        assert_eq!(
            env.env_vars.get("http_proxy").unwrap(),
            "http://127.0.0.1:1"
        );
        assert_eq!(
            env.env_vars.get("https_proxy").unwrap(),
            "http://127.0.0.1:1"
        );
        assert!(env.env_vars.get("no_proxy").unwrap().contains("localhost"));
    }
}

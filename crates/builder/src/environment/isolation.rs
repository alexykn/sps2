...
mod tests {
    ...
    #[tokio::test]
    async fn test_verify_environment_sanitization() {
        ...
        assert!(env.verify_environment_sanitization().is_ok());

        // Add dangerous variable
        env.env_vars
            .insert("LD_LIBRARY_PATH".to_string(), "/usr/local/lib".to_string());
        assert!(env.verify_environment_sanitization().is_err());

        // Remove it and verify again
        env.env_vars.remove("LD_LIBRARY_PATH");
        assert!(env.verify_environment_sanitization().is_ok());
    }

    #[tokio::test]
    async fn test_verify_compiler_flags() {
        let temp = tempdir().unwrap();
        let context = BuildContext::new(
            "test-pkg".to_string(),
            Version::parse("1.0.0").unwrap(),
        ...
        // Should have proper flags set
        assert!(env.verify_compiler_flags().is_ok());

        // Modify CFLAGS to include system path
        env.env_vars
            .insert("CFLAGS".to_string(), "-I/usr/local/include".to_string());
        assert!(env.verify_compiler_flags().is_err());
    }

    #[tokio::test]
    async fn test_verify_network_isolation() {
        let temp = tempdir().unwrap();
        let context = BuildContext::new(
            "test-pkg".to_string(),
            Version::parse("1.0.0").unwrap(),
        ...
        assert!(!env.verify_network_isolation().unwrap());

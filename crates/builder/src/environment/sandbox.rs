...
mod tests {
    ...
    fn test_sandbox_config_default() {
        ...
        assert!(!config.allow_sysctl);
        assert_eq!(config.resource_limits.cpu_time, Some(3600));
    }
    ...
    fn test_sandbox_profile_generation() {
        ...
        let config = SandboxConfig::default();
        let profile = SandboxProfile::from_config(&config, temp.path());

        assert!(profile.content.contains("(version 1)"));
        assert!(profile.content.contains("(deny default)"));
        assert!(profile.content.contains("(allow process-fork)"));
        assert!(!profile.content.contains("(allow network*)"));
    }
    ...
    #[test]
    fn test_path_accessibility() {
        let config = SandboxConfig::default();

        // System paths should be readable/executable
        let usr_bin = Path::new("/usr/bin/gcc");
        let access = check_path_accessibility(usr_bin, &config);
        assert!(access.readable);
        assert!(access.executable);
        assert!(!access.writable);

        // Random paths should not be accessible
        let home = Path::new("/Users/someone/file");
        let access = check_path_accessibility(home, &config);
        assert!(!access.readable);
        assert!(!access.writable);
        assert!(!access.executable);
    }
    ...
    #[test]
    fn test_minimal_build_sandbox() {
        let temp = tempdir().unwrap();
        let build_dir = temp.path().join("build");
        std::fs::create_dir_all(&build_dir).unwrap();

        let config = minimal_build_sandbox(&build_dir);

        // Build directory should be fully accessible
        let access = check_path_accessibility(&build_dir, &config);
        assert!(access.readable);
        assert!(access.writable);
        assert!(access.executable);
    }
    ...
    #[test]
    fn test_verify_sandbox_available() {
        // This test might fail in CI environments without sandbox-exec
        // Just verify the function doesn't panic
        let _ = SandboxExecutor::verify_sandbox_available();
    }
}

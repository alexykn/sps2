...
mod tests {
    ...
    async fn test_key_manager_initialization() {
        ...
        let mut key_manager = KeyManager::new(temp.path().join("keys"));

        // Test with invalid bootstrap key - should fail
        let invalid_bootstrap_key = "invalid_key";
        let result = key_manager
            .initialize_with_bootstrap(invalid_bootstrap_key)
            .await;
        assert!(result.is_err());

        // Test with valid minisign key format - should succeed
        let valid_bootstrap_key = "RWSGOq2NVecA2UPNdBUZykp1MLhfMmkAK/SZSjK3bpq2q7I8LbSVVBDm";
        let result = key_manager
            .initialize_with_bootstrap(valid_bootstrap_key)
            .await;
        assert!(result.is_ok());
    }
    ...
    async fn test_key_persistence() {
        ...
        // Ensure keys directory exists
        tokio::fs::create_dir_all(key_manager.keys_dir.clone())
            .await
            .unwrap();

        // Create a test trusted key
        let test_key = TrustedKey {
        ...
        key_manager
            .trusted_keys
            .insert("test_key".to_string(), test_key.clone());

        // Save and reload
        key_manager.save_trusted_keys().await.unwrap();
        key_manager.trusted_keys.clear();
        key_manager.load_trusted_keys().await.unwrap();

        // Check that the key was persisted
        assert!(key_manager.trusted_keys.contains_key("test_key"));
        assert_eq!(
            key_manager.trusted_keys["test_key"].public_key,
            "test_public_key"
        );
    }
}

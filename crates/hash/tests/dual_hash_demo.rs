...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ BLAKE3 hash parsing: {} -> {}",
        if parsed_blake3.is_blake3() {
            "BLAKE3"
        } else {
            "Unknown"
        },
        if parsed_blake3 == blake3_hash {
            "Match"
        } else {
            "Mismatch"
        }
    ...
    println!(
        "✅ xxHash parsing:   {} -> {}",
        if parsed_xxhash.is_xxhash128() {
            "xxHash128"
        } else {
            "Unknown"
        },
        if parsed_xxhash == xxhash {
            "Match"
        } else {
            "Mismatch"
        }
    ...
    let explicit_xxhash =
        Hash::hash_file_with_algorithm(&test_file, HashAlgorithm::XxHash128).await?;

    println!(
        "Explicit BLAKE3:  {} ({})",
        explicit_blake3.to_hex(),
        if explicit_blake3 == blake3_hash {
            "✅ Match"
        } else {
            "❌ Mismatch"
        }

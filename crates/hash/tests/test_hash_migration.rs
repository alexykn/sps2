...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("📄 Content preview: {:?}...", std::str::from_utf8(&test_data[..50])?);
    ...
    println!("BLAKE3 example:   {} ({} chars)", blake3_example, blake3_example.len());
    
    // Simulate xxHash 128-bit (32 hex chars = 16 bytes)  
    let xxhash_example = "1a2b3c4d5e6f7890abcdef1234567890";
    println!("xxHash128 example: {} ({} chars)", xxhash_example, xxhash_example.len());

...
async fn test_file_hashing() {
    ...
    let xxhash_explicit = Hash::hash_file_with_algorithm(&test_file, HashAlgorithm::XxHash128).await.unwrap();
    let blake3_explicit = Hash::hash_file_with_algorithm(&test_file, HashAlgorithm::Blake3).await.unwrap();
...
async fn test_performance_difference() {
    ...
    let _xxhash = Hash::hash_file_with_algorithm(&test_file, HashAlgorithm::XxHash128).await.unwrap();
    let xxhash_duration = start.elapsed();
    
    // Time BLAKE3
    let start = Instant::now();
    let _blake3 = Hash::hash_file_with_algorithm(&test_file, HashAlgorithm::Blake3).await.unwrap();
...
async fn test_invalid_hex() {
    ...
    assert!(Hash::from_hex("12345678901234567890123456789012345678901234567890123456789012345678901234567890").is_err()); // Too long
}
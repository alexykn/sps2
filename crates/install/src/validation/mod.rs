...
mod tests {
    ...
    #[tokio::test]
    async fn test_validate_sp_file_comprehensive() {
        let temp_path = std::path::Path::new("/tmp/nonexistent.sp");

        // This will fail because the file doesn't exist, but tests the function
        let result = validate_sp_file_comprehensive(temp_path, None).await;
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_validate_format_only() {
        let mut temp_file = NamedTempFile::with_suffix(".sp").unwrap();
        temp_file.write_all(b"test content").unwrap();

        // This will likely fail due to invalid format, but tests the function
        let result = validate_format_only(temp_file.path(), None).await;
        // Could be error or unknown format - either is fine for this test
        assert!(result.is_err() || result.unwrap() == PackageFormat::Unknown);
    }

    #[tokio::test]
    async fn test_create_validation_report() {
        let temp_path = std::path::Path::new("/tmp/nonexistent.sp");

        // This will fail because the file doesn't exist
        let result = create_validation_report(temp_path, None).await;
        assert!(result.is_err());
    }
    ...
    fn test_validation_presets() {
        ...
        let high_sec = ValidationPresets::high_security();
        let ci_cd = ValidationPresets::ci_cd();

        // Just test that they can be created - internal validation would require async
        assert_eq!(dev.get_context().validation_config.timeout_seconds, 600);
        assert_eq!(prod.get_context().validation_config.timeout_seconds, 300);
        assert_eq!(
            high_sec.get_context().validation_config.timeout_seconds,
            120
        );
        assert_eq!(ci_cd.get_context().validation_config.timeout_seconds, 60);
    }

    #[test]
    fn test_validation_report() {
        let result = ValidationResult::new(PackageFormat::PlainTar);
        let report = ValidationReport {
            file_path: "test.sp".to_string(),
            validation_result: result,
            validation_duration: std::time::Duration::from_secs(1),
            timestamp: std::time::SystemTime::now(),
        };

        let summary = report.summary();
        assert!(summary.contains("test.sp"));
        assert!(summary.contains("1.00s"));

        assert!(!report.is_successful()); // is_valid is false by default
        assert!(report.formatted_warnings().is_empty());
    }

    #[test]
    fn test_module_exports() {
        // Test that main types are available and can be constructed
        assert!(matches!(PackageFormat::PlainTar, PackageFormat::PlainTar));
        let _context = ValidationContext::new();
        let _result = ValidationResult::new(PackageFormat::ZstdCompressed);

        // Test that builder is available
        let _builder = ValidationPipelineBuilder::new();

        // Test that presets are available
        let _dev_preset = ValidationPresets::development();
    }
}

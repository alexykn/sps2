...
mod tests {
    ...
    #[tokio::test]
    async fn test_validate_zstd_parameters_invalid() {
        // Create a file that's not actually zstd compressed
        let mut temp_file = NamedTempFile::with_suffix(".sp").unwrap();
        temp_file.write_all(b"not zstd data").unwrap();

        let result = validate_zstd_parameters(temp_file.path()).await;
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_test_zstd_decompression_invalid() {
        // Create a file that's not actually zstd compressed
        let mut temp_file = NamedTempFile::with_suffix(".sp").unwrap();
        temp_file.write_all(b"not zstd data").unwrap();

        let result = test_zstd_decompression(temp_file.path()).await;
        assert!(result.is_err());
    }

    #[tokio::test]
    async fn test_validate_zstd_archive_content_invalid() {
        // Create a file that's not actually zstd compressed
        let mut temp_file = NamedTempFile::with_suffix(".sp").unwrap();
        temp_file.write_all(b"not zstd data").unwrap();

        let mut result = ValidationResult::new(PackageFormat::ZstdCompressed);

        // This should handle the error gracefully and add warnings
        let validation_result = validate_zstd_archive_content(temp_file.path(), &mut result).await;

        // Should succeed with warnings rather than failing

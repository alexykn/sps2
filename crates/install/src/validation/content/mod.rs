...
mod tests {
    ...
    #[test]
    fn test_validate_package_manifest_missing() {
        let mut result = ValidationResult::new(PackageFormat::PlainTar);
        result.manifest = None;

        let validation_result = validate_package_manifest(&mut result);
        assert!(validation_result.is_ok());
        assert!(!result.warnings.is_empty());
        assert!(result.warnings[0].contains("manifest.toml not found"));
    }

    #[test]
    fn test_validate_package_manifest_invalid() {
        let mut result = ValidationResult::new(PackageFormat::PlainTar);
        result.manifest = Some("invalid toml [[[".to_string());

        let validation_result = validate_package_manifest(&mut result);
        assert!(validation_result.is_ok());
        assert!(!result.warnings.is_empty());
        assert!(result.warnings[0].contains("manifest validation failed"));
    }

    #[test]
    fn test_validate_content_limits() {
        let result = ValidationResult {
            is_valid: true,
            format: PackageFormat::PlainTar,
            file_count: 100,
            extracted_size: 1024 * 1024, // 1MB
            warnings: Vec::new(),
            manifest: None,
        };

        let limits = ContentLimits::default();
        assert!(validate_content_limits(&result, &limits).is_ok());

        // Test exceeding limits
        let bad_result = ValidationResult {
            file_count: limits.max_files + 1,
            ..result
        };
        assert!(validate_content_limits(&bad_result, &limits).is_err());
    }

    #[tokio::test]
    async fn test_validate_archive_content_unknown_format() {
        let mut result = ValidationResult::new(PackageFormat::Unknown);
        let temp_path = std::path::Path::new("/tmp/nonexistent.sp");

        let validation_result =
            validate_archive_content(temp_path, &PackageFormat::Unknown, &mut result, None).await;

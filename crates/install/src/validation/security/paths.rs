...
mod tests {
    ...
    fn test_validate_safe_path() {
        // Valid paths
        assert!(validate_safe_path("some/normal/path.txt").is_ok());
        assert!(validate_safe_path("file.txt").is_ok());
        assert!(validate_safe_path("dir/").is_ok());

        // Invalid paths
        assert!(validate_safe_path("../etc/passwd").is_err());
        assert!(validate_safe_path("/absolute/path").is_err());
    ...
    fn test_validate_path_component() {
        // Valid components
        assert!(validate_path_component("normal_file.txt").is_ok());
        assert!(validate_path_component("dir").is_ok());

        // Invalid components
        assert!(validate_path_component("").is_err());
        assert!(validate_path_component("CON").is_err());
        assert!(validate_path_component("COM1").is_err());
        assert!(validate_path_component("PRN.txt").is_err());

        // Long component
        let long_name = "a".repeat(256);
        assert!(validate_path_component(&long_name).is_err());
    }

    #[test]
    fn test_normalize_path() {
        assert_eq!(normalize_path("some/./path"), PathBuf::from("some/path"));
        assert_eq!(normalize_path("./file.txt"), PathBuf::from("file.txt"));
        assert_eq!(normalize_path("normal/path"), PathBuf::from("normal/path"));
    }

    #[test]
    fn test_path_security_policy() {
        let policy = PathSecurityPolicy::new().with_blocked_pattern("dangerous".to_string());

        assert!(policy.validate_path("safe/path").is_ok());
        assert!(policy.validate_path("path/with/dangerous/content").is_err());
    }

    #[test]
    fn test_dangerous_patterns() {
        let patterns = get_dangerous_path_patterns();

...
mod tests {
    ...
    fn test_security_policies() {
        ...
        let paranoid = SecurityPolicy::paranoid();
        assert_eq!(paranoid.security_level, SecurityLevel::Paranoid);
        assert!(paranoid.is_strict());
    }

    #[test]
    fn test_path_matches_pattern() {
        assert!(path_matches_pattern("file.txt", "*.txt"));
        assert!(path_matches_pattern("script.sh", "*.sh"));
        assert!(!path_matches_pattern("file.txt", "*.sh"));

        assert!(path_matches_pattern("temp/file", "temp*"));
        assert!(path_matches_pattern("test_file", "test"));
    }

    #[test]
    fn test_custom_security_rule() {
        let rule = CustomSecurityRule {
            name: "test_rule".to_string(),
            description: "Test rule".to_string(),
            is_fatal: true,
            pattern: SecurityPattern::PathPattern("*.exe".to_string()),
        };

        assert_eq!(rule.name, "test_rule");
        assert!(rule.is_fatal);
    }
    ...
    fn test_security_violation() {
        ...
        .with_context("file".to_string(), "test.txt".to_string());

        assert!(violation.is_fatal());
        assert_eq!(violation.context.get("file"), Some(&"test.txt".to_string()));
    }
    ...
    fn test_validate_file_path() {
        ...
        // Valid path
        assert!(policy.validate_file_path("valid/path.txt").is_ok());

        // Invalid path
        assert!(policy.validate_file_path("../escape").is_err());
    }

    #[test]
    fn test_security_level_descriptions() {
        let policy = SecurityPolicy::standard();
        assert!(policy.security_level_description().contains("Standard"));

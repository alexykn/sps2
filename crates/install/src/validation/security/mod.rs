...
mod tests {
    ...
    fn test_validate_file_security() {
        ...
        // Valid file
        let result =
            validate_file_security("valid/path.txt", 1024, Some(0o644), false, None, &policy);
        assert!(result.is_ok());

        // Dangerous symlink
        let result = validate_file_security("link", 0, None, true, Some("../escape"), &policy);
        assert!(result.is_err());
    }

    #[test]
    fn test_security_report() {
        let mut report = SecurityReport::new(SecurityLevel::Standard);
        assert_eq!(report.overall_status, SecurityStatus::Secure);
        assert!(report.is_safe_to_install());

        report.add_warning("test warning".to_string());
        assert_eq!(report.overall_status, SecurityStatus::Warning);
        assert!(report.is_safe_to_install());
        ...
        assert_eq!(report.overall_status, SecurityStatus::Dangerous);
        assert!(!report.is_safe_to_install());
    }

    #[test]
    fn test_apply_security_policy_to_results() {
        let mut result = ValidationResult::new(PackageFormat::PlainTar);
        result.extracted_size = 1024;

        let policy = SecurityPolicy::standard();
        assert!(apply_security_policy_to_results(&mut result, &policy).is_ok());
    }

    #[tokio::test]
    async fn test_validate_package_security() {
        let mut result = ValidationResult::new(PackageFormat::PlainTar);
        let policy = SecurityPolicy::standard();
        let temp_path = std::path::Path::new("/tmp/nonexistent.sp");

        // This will fail because the file doesn't exist, but tests the function signature
        let validation_result = validate_package_security(
            temp_path,
        ...
        assert!(validation_result.is_err()); // Expected because file doesn't exist
    }
}

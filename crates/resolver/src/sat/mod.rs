...
mod tests {
    ...
    fn test_package_version() {
        ...
        assert_eq!(pv.to_string(), "test@1.0.0");
    }

    #[test]
    fn test_dependency_problem() {
        ...
        let _v1 = problem.add_package_version(pv1);
        let _v2 = problem.add_package_version(pv2);

        problem.require_package("foo".to_string());

        assert_eq!(problem.get_package_versions("foo").len(), 2);
        assert!(problem.required_packages.contains("foo"));
    }
    ...
    async fn test_simple_resolution() {
        ...
        // Add foo versions
        let foo1 = PackageVersion::new("foo".to_string(), Version::parse("1.0.0").unwrap());
        let foo2 = PackageVersion::new("foo".to_string(), Version::parse("2.0.0").unwrap());

        let v1 = problem.add_package_version(foo1);
        let _v2 = problem.add_package_version(foo2);

        // Require foo
        problem.require_package("foo".to_string());
        problem.add_at_least_one_constraint("foo");
        problem.add_at_most_one_constraint("foo");

        // Add preference for version 1.0.0 (just positive literal)
        problem.add_clause(Clause::new(vec![Literal::positive(v1)]));

        let solution = solve_dependencies(problem).await.unwrap();

        assert_eq!(
            solution.selected.get("foo").unwrap(),

...
mod tests {
    ...
    fn test_dimacs_conversion() {
        ...
        assert_eq!(pos.to_dimacs(), 1);
        assert_eq!(neg.to_dimacs(), -1);

        assert_eq!(Literal::from_dimacs(1), pos);
        assert_eq!(Literal::from_dimacs(-1), neg);
    }
    ...
    fn test_assignment() {
        ...
        assert_eq!(assignment.get(v1), TruthValue::Unassigned);
        assert!(!assignment.is_assigned(v1));

        assignment.assign(v1, true, 1);
        assert!(assignment.is_true(v1));
        assert_eq!(assignment.level(v1), Some(1));

        assignment.assign(v2, false, 2);
        assert!(assignment.is_false(v2));
        assert_eq!(assignment.level(v2), Some(2));

        assert_eq!(assignment.num_assigned(), 2);

        // Test literal evaluation
        let lit1 = Literal::positive(v1);
        let lit2 = Literal::negative(v2);

        assert_eq!(assignment.eval_literal(lit1), TruthValue::True);
        assert_eq!(assignment.eval_literal(lit2), TruthValue::True);

        // Test backtracking
        assignment.backtrack_to(1);
        assert!(assignment.is_assigned(v1));
        assert!(!assignment.is_assigned(v2));

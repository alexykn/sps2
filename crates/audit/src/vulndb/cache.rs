...
mod tests {
    ...
    fn test_cache_creation() {
        let cache = VulnerabilityCache::with_default_ttl();
    ...
    fn test_package_caching() {
        let cache = VulnerabilityCache::with_default_ttl();
        let key = "test-package".to_string();
        let vulns = vec![]; // Empty vulnerabilities for test

        // Cache should be empty initially
        assert!(cache.get_package_vulnerabilities(&key).is_none());

        // Cache the vulnerabilities
        cache.cache_package_vulnerabilities(key.clone(), vulns);

        // Should now be cached (just checking that it's not None)
        assert!(cache.get_package_vulnerabilities(&key).is_some());
    ...
    fn test_cache_clearing() {
        ...
        cache.cache_package_vulnerabilities("test".to_string(), vec![]);
        cache.cache_purl_vulnerabilities("pkg:test".to_string(), vec![]);

        let stats = cache.get_cache_stats();
        assert_eq!(stats.package_entries, 1);
        assert_eq!(stats.purl_entries, 1);

        cache.clear();

        let stats = cache.get_cache_stats();
        assert_eq!(stats.package_entries, 0);
        assert_eq!(stats.purl_entries, 0);
    }
}
